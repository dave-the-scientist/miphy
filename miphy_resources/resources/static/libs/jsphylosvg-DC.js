...
Smits.PhyloCanvas.Render.Phylogram.prototype = {
	...
	function recursiveCalculateNodePositions(node, positionX){
		...
		if(node.children.length > 0){
			...
			if(node.name){
				// DAVE CURRAN EDITS BEGIN:
				treeDrawingParams['seqs'].push([node.name, y1]);
				// DAVE CURRAN EDITS END.
	...
	return function(sSvg, dataObject, bufferRadius){
		...
		// DAVE CURRAN EDITS BEGIN: added sParams.bufferInnerLabels.
		outerRadius += sParams.bufferInnerLabels - 4;
		// DAVE CURRAN EDITS END.
		...
		treeDrawingParams['initStartAngle'] = initStartAngle;
		treeDrawingParams['cx'] = cx;
		treeDrawingParams['cy'] = cy;
		// DAVE CURRAN EDITS END.
