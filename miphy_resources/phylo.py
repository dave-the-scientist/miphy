...
# TODO:
# Integrate this with MIPhy and Navargator
#  - For MIPhy: just need gene_children (self.get_named_children()), species_paths (self.get_named_paths()), gene_root (self.root.name), and gene_leaves (self.get_named_leaves()). for non-huge data sets, it also needs the coordinate points. There's also some elementtree processing of phyloxml going on in the Tree class of miphy_instance.py; not sure if I should move that here (I'm thinking no, as it's pretty specific, but could fit with my desire for an annotation format).
#  - For Navargator: needs root (self.root.name), leaves (in an ordered list as with MIPhy), index (dict[name] = leaves.index(name); navargator can calculate that itself), dist (numpy matrix of pairwise distances)

# Decide to keep this as a standalone (probably not) or include it as part of molecbio. Regardless, update the import calls in miphy (and add molecbio as a dependency).
