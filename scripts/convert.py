        ...
        while i < len(newick_str):
            ...
            if lj == j: # new internal node.
                ...
                if r_prev: # complete latest internal node.
                    ...
                    if name: self.leaves.append(name)
            ...
            if j == final_r:
                ...
                break
            """ Not sure why I originally wrote as below, but it appears to be wrong. This is left just in case I did have a good reason.
            if rj == final_r: # we're at the end.
                name,_,branch = newick_str[j+1:final_r].partition(':')
                name = name.strip()
                degree = len(top)
                if degree < self.degree[0]: self.degree[0] = degree
                if degree > self.degree[1]: self.degree[1] = degree
                self._add_name_branch(top, name, branch)
                self._find_node_degree(top)
                break
            """
            if rj == j: r_prev = True
